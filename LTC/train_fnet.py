...
if Material == 'TF':
    ...
    data = h5py.File('data/data391.mat', 'r')
    StartWL = 6.75
    EndWL = 16.76
    Resolution = 0.05
    WL = np.arange(StartWL, EndWL, Resolution)
    OutputNum = WL.size
    Input_data = torch.tensor(data['parameter'][:, 0:DataSize], device=device_data, dtype=dtype).t()
    Output_data = torch.tensor(data['pcr'][:, 0:DataSize], device=device_data, dtype=dtype).t()
    idx = torch.randperm(DataSize)
    Input_data = Input_data[idx, :]
    Output_data = Output_data[idx, :]
    Input_train = Input_data[0:DataSize, :]
    Output_train = Output_data[0:DataSize, :]
...
optimizer = AdaBelief(fnet.parameters(), lr=1e-3, eps=1e-12, betas=(0.9, 0.999), weight_decouple=True, rectify=False)
